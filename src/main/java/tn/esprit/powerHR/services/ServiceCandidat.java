...
public class ServiceCandidat implements IService<Candidat> {
    ...
    public void add(Candidat candidat) {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            String filePath = candidat.getFilePath();

            // 🔴 Debug: Print the file path to check if it's null/empty
            System.out.println("File path received in add(): " + filePath);

            if (filePath != null && !filePath.isEmpty()) {
                String cvPdfUrl = MinIOUtils.uploadCV("cv-bucket", filePath, new File(filePath).getName());
                System.out.println("Generated CV URL: " + cvPdfUrl); // Debug
